...
public class ThreeBulletMove : MonoBehaviour {
	...
	void Update () {
		...
		if (istrigger) {
			rigid1.velocity = new Vector2 (-2, 2f);
			rigid2.velocity = new Vector2 (0, 2f);
			rigid3.velocity = new Vector2 (2, 2f);
		} 
	}
	public void OnTriggerEnter2D(Collider2D  other){
		if (other.name == "bu") {
			...
			istrigger = true;
			rigid1 = ((GameObject)GameObject.Instantiate (bullect, new Vector2 (0, 2),Quaternion.Euler(0,0,45))).GetComponent<Rigidbody2D> ();
			rigid1.gameObject.transform.SetParent(myparent.transform,false);
			//Destroy (rigid1.gameObject.GetComponent<BoxCollider2D> ());
			//rigid1.name="r1";
	

			rigid2 = ((GameObject)GameObject.Instantiate (bullect, new Vector2 (0, 2), Quaternion.identity)).GetComponent<Rigidbody2D> ();
